...
public class CryptoUtils {
    ...
    public static KeyStorePasswordPair getKeyStorePasswordPair(final String certificateFile, final String privateKeyFile, String keyAlgorithm) {
        ...
        try    {
    		...
    		Path path = Paths.get(Constantes.privateKeyCertificadoDevice);
            ...
            System.err.println ("Excepcion al recuperar la clave privada");
            return null;
        }
    ...
    public static Certificate loadCertificateFromFile(final String filename) {
        File file = new File(filename);
        if (!file.exists()) {
            //System.out.println("Certificate file: " + filename + " is not found.");
            return null;
        ...
        try (BufferedInputStream stream = new BufferedInputStream(new FileInputStream(file))) {
            final CertificateFactory certFactory = CertificateFactory.getInstance("X.509");
            return (Certificate) certFactory.generateCertificate(stream);
        } catch (IOException | CertificateException e) {
            System.out.println("Failed to load certificate file " + filename);
            e.printStackTrace();
        }
